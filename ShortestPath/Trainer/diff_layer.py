...
def SPOlayer(solver, alpha=2, minimize=True):
...
def SPOnormLayer(solver, alpha=2, minimize=True):
    mm = 1 if minimize else -1
    class SPOlayer_cls(torch.autograd.Function):
        @staticmethod
        def forward(ctx, y_hat,y_true,sol_true ):
...
def DBBlayer(solver,lambda_val=1., minimize = True):
    mm = 1 if minimize else -1
    class DBBlayer_cls(torch.autograd.Function):
        ...
        def backward(ctx, grad_output):
            ...
            return -mm*(sol_hat - sol_perturbed), None, None

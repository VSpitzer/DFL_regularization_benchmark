"""
Best-Results Summary for the Shortest Path Benchmark

Reads the CSV files produced in 'Rslt/' by test_sp.py (one file per
model/loss combination, named "{model}{loss}.csv"). Unlike Matching and
instability_problem, ShortestPath does NOT encode the problem degree ('deg')
in the filename: a single file mixes all 'deg' values (1, 2, 6) run for that
model, distinguished only by the 'deg' column inside the CSV. This script
accounts for that by grouping separately per 'deg' value found in each file,
so the summary still reports one best hyperparameter combination per
(model, deg) pair.

For every model and every degree, this reports the hyperparameter
combination with the lowest mean validation regret across the 10 seeds,
together with its corresponding mean test regret.

This script is only meaningful for a '--config config_grid.json' run (the
full hyperparameter grid sweep): any rows in Rslt/ that came from a different
config (e.g. a '--config config.json' tuned-hyperparameters run) are silently
...
Selection rule: for each (model, deg), hyperparameter combinations are
...
Alongside the mean, the table also reports 'test_regret_std' and
'test_mse_std' -- the standard deviation of that same best combination's
test-set metrics across its seeds -- so the reported test performance can be
read together with how much it varies seed-to-seed, not just its average.
"""
...
def summarize_file(path):
    ...
    if "config" in df.columns:
        df = df[df["config"] == "config_grid.json"]
        if df.empty:
            return [], os.path.basename(path), None, note

    hp_cols = [c for c in df.columns if is_hyperparam_col(c)]
    metric_cols = [c for c in ("val_regret", "test_regret", "val_mse", "test_mse") if c in df.columns]
    ...
    # Dispersion across seeds for the test-set metrics specifically (the
...
def main():
    parser = argparse.ArgumentParser(
        description="Print the best hyperparameter configuration (by mean validation "
                    "regret across seeds) for each model/degree in a Rslt/ directory "
                    "produced by test_sp.py."
    )
    parser.add_argument("--rslt_dir", type=str, default="Rslt",
                         help="Directory containing the result CSVs produced by test_sp.py (default: Rslt)")
    ...
    if not files:
        print("No result files found in '{}'. Run test_sp.py first.".format(args.rslt_dir))
        return
    ...
    front_cols = [c for c in ("model", "instance", "deg", "lr", "kappa", "alpha", "lambda_val", "sigma",
                              "num_samples", "val_regret", "test_regret", "test_regret_std", "val_mse",
                              "test_mse", "test_mse_std", "n_seeds") if c in summary.columns]
    ...
    print("\nBest hyperparameters per model (selected on lowest mean val_regret across {} seeds; "
          "test_regret_std/test_mse_std are the std dev of that combination's test metrics across seeds):\n".format(

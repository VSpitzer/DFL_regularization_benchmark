...
relu_net_layers = [nn.Linear(5,40), nn.ReLU()]
relu_net = nn.Sequential(*relu_net_layers)
...
network_dict = {"nonorm":nonorm_net, "batchnorm":batchnorm_net, "l1norm":l1norm_net, "relu": relu_net}
...
def exec():
    ...
    for parameters in parameter_sets:
        ...
        for seed in range(10):
            ...
            trainer = pl.Trainer(max_epochs= argument_dict['max_epochs'],callbacks=[checkpoint_callback],  min_epochs=5, logger=tb_logger, check_val_every_n_epoch=25)
            ...
            regret_list_data = regret_list(spsolver, y_pred, torch.from_numpy(y_test).float(), sol_test)

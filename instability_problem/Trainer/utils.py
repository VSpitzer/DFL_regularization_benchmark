...
def batch_solve(solver,y,m,batched= True):
    ...
    if batched:
        ...
        for i in range(len(y)):
            sol.append(  solver.solve(y[i].detach().numpy(), m[i].numpy()) )
        return torch.tensor(sol).float()
    else:
        ### y, m both are of dim (2500)
        sol = solver.solve(y.detach().numpy(), m.numpy())
        return torch.tensor(sol).float()
...
def abs_regret_list(solver,y_hat,y_true,sol_true,m,minimize=False):
    mm = 1 if minimize else -1    
    sol_hat = batch_solve(solver, y_hat,m)
...
def abs_regret_fn(solver,y_hat,y_true,sol_true,m,minimize=False):
    # mm = 1 if minimize else -1    
    # sol_hat = batch_solve(y_hat,m)
    # sol_ = batch_solve(y,m)
    # # return ((mm*(sol_hat - sol_)*y).sum(1)/ (sol_*y).sum(1) ).mean()
    return  abs_regret_list(solver,y_hat,y_true,sol_true,m,minimize=minimize).mean()

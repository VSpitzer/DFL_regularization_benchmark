"""
Best-Results Summary for the Bipartite Matching Benchmark

Reads the CSV files produced in 'Rslt/' by test_matching.py (one file per
model/loss/instance combination, named "{model}matching{loss}{instance}.csv")
and reports, for every model and every instance, the hyperparameter
...
def main():
    parser = argparse.ArgumentParser(
        description="Print the best hyperparameter configuration (by mean validation "
                    ...
                    "produced by test_matching.py."
    )
    parser.add_argument("--rslt_dir", type=str, default="Rslt",
                         help="Directory containing the result CSVs produced by test_matching.py (default: Rslt)")
    ...
    if not files:
        print("No result files found in '{}'. Run test_matching.py first.".format(args.rslt_dir))
        return
